// VITAFIT 3.6.8 — rotina nativa 4x/semana, sem remapeamento visual
(function(){
  const BUILD='3.6.8';
  const NS='shape12.v360.';
  const dayOrder=['seg','qua','sex','dom'];
  const dayMeta={
    seg:{dow:1,label:'SEG',title:'Inferiores completo',subtitle:'Inferiores • musculação'},
    qua:{dow:3,label:'QUA',title:'Superiores completo',subtitle:'Superiores • musculação'},
  ...
  const nextWorkoutByDow={0:'dom',1:'seg',2:'qua',3:'qua',4:'sex',5:'sex',6:'dom'};
  ...
  function selectedDow(){try{return (window.selectedDate||new Date()).getDay()}catch(e){return new Date().getDay()}}
  function initialDay(){const d=nextWorkoutByDow[selectedDow()]||'seg';return plan[d]?d:'seg'}
  ...
  function shell(){
    ...
    if(!training.classList.contains('v360-ready')){
      ...
      const toast=document.createElement('div');toast.id='v360Toast';toast.className='v360-toast';document.body.appendChild(toast);bindApp(app);
    }
    return document.getElementById('v360TrainingApp');
  }
  function render(){
    const app=shell();if(!app)return;const meta=dayMeta[activeDay],p=progress();
    app.innerHTML=`<section class="v360-hero"><span class="v360-kicker">ROTINA SEMANAL • 4 TREINOS</span><h1>Treino da semana</h1><p>Musculação segunda, quarta, sexta e domingo. Terça, quinta e sábado ficam somente para cardio e recuperação ativa.</p></section><div class="v360-week">${dayOrder.map(d=>`<button class="v360-day ${d===activeDay?'active':''}" data-day="${d}"><b>${dayMeta[d].label}</b><span>${dayMeta[d].title.replace(' completo','')}</span></button>`).join('')}</div><section class="v360-overview"><div><span class="v360-kicker">${meta.label} • TREINO DO DIA</span><h2>${meta.title}</h2><p>${meta.subtitle}</p><div class="v360-badges"><span class="v360-badge">${plan[activeDay].length} exercícios</span><span class="v360-badge">${p.total} séries</span></div></div><div class="v360-progress" style="--p:${p.pct}%"><b>${p.pct}%</b></div></section><div class="v360-list">${plan[activeDay].map(exCard).join('')}</div>`;
    syncBuild();
  }
  ...
  function toggleSet(btn){
    const exKey=btn.dataset.done,i=Number(btn.dataset.set),k=key(exKey,i,'done'),val=!json(k,false);put(k,val);saveHistory(exKey);btn.classList.toggle('done',val);
    const ex=findExercise(exKey),card=btn.closest('[data-ex-card]');if(ex&&card)card.classList.toggle('done',exerciseDone(ex));updateProgressOnly();showToast(val?'Série concluída':'Série reaberta');
  }
  ...
  window.currentWorkoutDetails=function(){
    ...
    if(dow===2||dow===4||dow===6)return {id:null,name:'Cardio',tag:'Recuperação ativa'};
    const d=dayOrder.find(x=>dayMeta[x].dow===dow);return d?{id:'workout-'+d,name:dayMeta[d].title,tag:dayMeta[d].subtitle}:{id:null,name:'Recuperação',tag:'Descanso'};
  };
