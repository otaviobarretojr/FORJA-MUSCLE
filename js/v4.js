...
function mount(){
 ...
 bindGlobal();renderAll();syncBuild();document.documentElement.dataset.vitafitReady='true';
}
function bindGlobal(){
...
function progressStrength(day){let total=0,done=0;(PLAN[day]||[]).forEach(ex=>{total+=ex.sets;for(let i=1;i<=ex.sets;i++)if(read(NS,`day.${iso(targetDateForDow(trainingDow))}.${ex.k}.${i}.done`,false))done++});return {total,done,pct:total?Math.round(done/total*100):0}}
...
function renderAll(){renderHome();renderTraining();renderMore()}
