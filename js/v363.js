...
(function(){
  ...
  function toast(t){let e=document.getElementById('v363Toast');if(!e){e=document.createElement('div');e.id='v363Toast';e.className='v363-toast';document.body.appendChild(e)}e.textContent=t;e.classList.add('show');clearTimeout(e._t);e._t=setTimeout(()=>e.classList.remove('show'),1200)}
  async function apply(force=false){
    ...
    const day=activeDay(),token=++applyToken;
    if(!force&&overview.dataset.localVideoDay===day&&overview.querySelector('#v363VideoBox'))return;
    const saved=await getVideo(day);if(token!==applyToken)return;
    overview.dataset.localVideoDay=day;overview.querySelector('#v363VideoBox')?.remove();
    const box=document.createElement('div');box.id='v363VideoBox';box.className='v363-video-box';
    if(saved){
      ...
      box.querySelector('.v363-main').onclick=()=>openVideo(day);box.querySelector('[data-act="replace"]').onclick=()=>chooseVideo(day);box.querySelector('[data-act="remove"]').onclick=async()=>{if(!confirm('Remover o vídeo salvo deste treino neste aparelho?'))return;await delVideo(day);toast('Vídeo removido');overview.dataset.localVideoDay='';apply(true)};
    }else{
      box.innerHTML='<button class="v363-main empty" type="button"><span class="v363-play">＋</span><span><b>Importar vídeo da galeria</b><small>Será salvo localmente, na qualidade original</small></span><span class="v363-arrow">›</span></button>';box.querySelector('.v363-main').onclick=()=>chooseVideo(day);
    }
    (overview.firstElementChild||overview).appendChild(box);
  }
  const prevSet=window.setAppScreen;if(typeof prevSet==='function')window.setAppScreen=function(id,opts={}){prevSet(id,opts);if(id==='treino')requestAnimationFrame(()=>apply())};
  document.addEventListener('vitafit-screen-change',e=>{if(e.detail?.id==='treino')requestAnimationFrame(()=>apply())});
  document.addEventListener('click',e=>{if(e.target.closest('#v360TrainingApp .v360-day'))requestAnimationFrame(()=>apply(true))},{passive:true});
