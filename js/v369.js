// VITAFIT 3.6.10 — polimento visual sem adicionar complexidade
(function(){
  const BUILD='3.6.10';
  const CARDIO={2:{label:'TER',minutes:20},4:{label:'QUI',minutes:40},6:{label:'SÁB',minutes:20}};
  const STRENGTH={1:{label:'SEG',day:'seg',title:'Inferiores completo',short:'Inferiores'},3:{label:'QUA',day:'qua',title:'Superiores completo',short:'Superiores'},5:{label:'SEX',day:'sex',title:'Inferiores completo',short:'Inferiores'},0:{label:'DOM',day:'dom',title:'Superiores completo',short:'Superiores'}};
  ...
  const dateKeyFrom=d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
  const targetDateForDow=dow=>{const base=new Date(dateObj());base.setHours(12,0,0,0);const pos=x=>x===0?6:x-1;base.setDate(base.getDate()+(pos(dow)-pos(base.getDay())));return base};
  const cardioKey=dow=>`cardio.${dateKeyFrom(targetDateForDow(dow))}`;
  ...
  function ensureWeek(){
    ...
    strip.innerHTML=ORDER.map(d=>{
      const s=STRENGTH[d],c=CARDIO[d],isToday=d===today;
      return `<button type="button" class="v369-day ${s?'strength':'cardio'} ${isToday?'today':''}" data-v369-dow="${d}" aria-pressed="false"><b>${s?s.label:c.label}</b><span>${s?'Treino':'Cardio'}</span><small>${s?s.short:c.minutes+' min'}</small></button>`
    }).join('');
  ...
  function markSelected(dow){document.querySelectorAll('#v369Week .v369-day').forEach(b=>{const selected=Number(b.dataset.v369Dow)===dow;b.classList.toggle('selected',selected);b.setAttribute('aria-pressed',String(selected))})}
  ...
  function showCardio(dow){
    ...
    const done=get(cardioKey(dow),false);
    panel.innerHTML=`<span class="v369-kicker">${DAYNAME[dow].toUpperCase()} • CARDIO</span><h2>${c.minutes} min de cardio</h2><p>Ritmo confortável e sustentável.</p><div class="v369-cardio-meta"><div><b>${c.minutes}</b><span>minutos</span></div><div><b>${done?'100%':'0%'}</b><span>concluído</span></div></div><button type="button" id="v369CardioDone" class="${done?'done':''}">${done?'✓ Cardio concluído':'Marcar cardio concluído'}</button>`;
    panel.querySelector('#v369CardioDone')?.addEventListener('click',()=>{const val=!get(cardioKey(dow),false);put(cardioKey(dow),val);showCardio(dow);syncDashboard()});
  ...
  function syncDashboard(){
    ...
    if(c){
      const done=get(cardioKey(dow),false);
      if(title)title.textContent='Cardio';if(sub)sub.textContent=`${DAYNAME[dow]} • ${c.minutes} min`;
      if(a)a.textContent=String(c.minutes);if(al)al.textContent='minutos';if(b)b.textContent=done?'100%':'0%';if(bl)bl.textContent='concluído';if(cc)cc.textContent='Cardio';if(cl)cl.textContent='atividade';
      ...
      if(eyebrow)eyebrow.textContent='HOJE';if(cardioTitle)cardioTitle.textContent='Cardio';if(cardioDetail)cardioDetail.textContent=`${c.minutes} min programados`;
    }else if(s){
      if(title)title.textContent=s.title;if(sub)sub.textContent=`${DAYNAME[dow]} • musculação`;if(eyebrow)eyebrow.textContent='HOJE';if(cardioTitle)cardioTitle.textContent='Treino';if(cardioDetail)cardioDetail.textContent=s.short;if(action)action.textContent='▶ INICIAR TREINO';
    }
  }
