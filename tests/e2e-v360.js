...
(async()=>{
  ...
  try{
    ...
    await page.goto('http://127.0.0.1:4173/?e2e=v364',{waitUntil:'domcontentloaded'});
    await page.waitForFunction(()=>document.documentElement.dataset.vitafitReady==='true'&&document.getElementById('v360TrainingApp'));

    const boot=await page.evaluate(()=>({build:document.documentElement.dataset.vitafitBuild,nav:document.querySelectorAll('.app-nav-btn').length,programNav:!!document.querySelector('.app-nav-btn[data-screen="evolucao"]'),programHidden:document.getElementById('evolucao')?.hidden===true,legacyHidden:document.getElementById('v360LegacyTraining')?.parentElement?.classList.contains('v360-ready')||false}));
    assert.equal(boot.build,'3.6.4');assert.equal(boot.nav,4);assert.equal(boot.programNav,false);assert.equal(boot.programHidden,true);assert.equal(boot.legacyHidden,true);
    ...
    console.log('VITAFIT 3.6.4 simplified browser audit: OK');
